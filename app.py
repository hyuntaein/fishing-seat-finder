...
# 어종과 낚시방식을 분리.
FISH_OPTIONS = ["전체", "문어", "주꾸미", "갑오징어", "광어", "우럭", "참돔", "갈치", "농어", "백조기", "대구", "삼치", "기타"]
METHOD_OPTIONS = ["전체", "타이라바", "다운샷", "외수질", "생미끼", "지깅", "캐스팅", "텐야", "팁런", "기타"]

# 어종별 참고용 생태/낚시 정보 (API 연동 대신 고정 데이터로 관리)
FISH_INFO = {
    "참돔": {
        "제철": "5월~11월 초(서해 기준, 제주는 4~12월로 더 김)",
        "서식지": "수심 20~100m 암초·모래 지형, 조류 흐름이 있는 곳",
        "낚시방법": "타이라바(러버지깅)가 대표적, 활성 낮을 땐 갯지렁이 병행",
        "주요 포인트": "군산·보령·인천(서해), 여수·부산(남해), 제주 서귀포",
        "특징": "입질이 예민해 초리 감도 좋은 연질 로드 선호, 바닥층 위주로 낚임",
    },
    "광어": {
        "제철": "가을~초겨울(9~12월)이 살이 많이 올라 인기, 사계절 조황 있음",
        "서식지": "모래·펄 바닥 저서성 어종, 연안 가까운 얕은 수심대",
        "낚시방법": "다운샷 리그가 대표적, 웜/미노우 활용",
        "주요 포인트": "서해 전 지역(태안·보령·군산 등), 남해안 일부",
        "특징": "바닥에 붙어 위장하는 습성, 입질 시 살짝 끌어주다 챔질",
    },
    "우럭": {
        "제철": "사계절 가능, 여름철에 활성 좋은 편",
        "서식지": "암초·구조물 주변, 근해 여러 수심대",
        "낚시방법": "다운샷, 생미끼(밴댕이·오징어 등) 병행",
        "주요 포인트": "서해·남해 전 지역 방파제/근해 공통",
        "특징": "은신처를 좋아해 구조물 주변을 집중 공략",
    },
    "문어": {
        "제철": "가을~초겨울(9~12월) 씨알 굵어짐",
        "서식지": "암초 지형, 은신할 수 있는 바위 틈",
        "낚시방법": "문어단지, 에깅류 문어 전용 채비",
        "주요 포인트": "서해·남해 암초 지대 두루 분포",
        "특징": "야행성이라 새벽·저녁 활성이 좋은 편",
    },
    "주꾸미": {
        "제철": "봄(3~5월) 산란철, 가을(9~10월)도 시즌",
        "서식지": "펄 바닥, 연안 얕은 수심",
        "낚시방법": "에깅(에기 채비), 무늬오징어와 비슷한 방식",
        "주요 포인트": "서해 보령·태안 일대가 대표 산지",
        "특징": "산란기엔 소라껍데기에 알을 낳는 습성 활용한 채집도 유명",
    },
    "갑오징어": {
        "제철": "봄(4~6월) 산란철이 절정",
        "서식지": "연안 암초·모래 혼합 지형",
        "낚시방법": "에깅, 갑오징어 전용 에기",
        "주요 포인트": "서해·남해 전 연안",
        "특징": "먹물을 뿜는 습성, 산란철 씨알이 굵어짐",
    },
    "갈치": {
        "제철": "여름~가을(7~10월) 성수기, 제주는 겨울도 시즌",
        "서식지": "표층~중층 회유성 어종, 야간에 표층 접근",
        "낚시방법": "지깅, 루어(메탈지그), 생미끼 병행",
        "주요 포인트": "제주 전역, 남해 여수·거문도",
        "특징": "야간 조황이 좋아 야간 출조가 많음, 이빨이 날카로워 채비 주의",
    },
    "농어": {
        "제철": "여름~가을 활성 최고조",
        "서식지": "연안 얕은 수심, 하구·갯바위 주변",
        "낚시방법": "외수질(웜 채비), 미노우 루어",
        "주요 포인트": "서해·남해 갯바위, 하구 인근",
        "특징": "포식성이 강해 표층 루어에 반응 좋음",
    },
    "백조기": {
        "제철": "여름철(6~8월) 성수기",
        "서식지": "연안 모래·펄 바닥",
        "낚시방법": "생미끼 또는 다운샷",
        "주요 포인트": "서해 전 지역",
        "특징": "무리지어 다니는 습성, 한번 포인트 잡으면 마릿수 조황",
    },
    "대구": {
        "제철": "겨울철(12~2월) 산란철",
        "서식지": "깊은 수심(50m 이상)의 찬물",
        "낚시방법": "생미끼, 무거운 봉돌 채비",
        "주요 포인트": "동해·남해 깊은 수심대",
        "특징": "저수온을 좋아해 겨울철 대표 어종",
    },
    "삼치": {
        "제철": "가을(8월 말~11월)이 낚시철, 식용은 겨울(12~1월)이 기름지고 맛 좋음",
        "서식지": "표층~중층을 빠르게 회유하는 등푸른 생선",
        "낚시방법": "지깅(메탈지그), 루어 캐스팅, 생미끼도 가능",
        "주요 포인트": "서해 시화호 인근, 남해 전역",
        "특징": "이빨이 날카롭고 힘이 좋아 와이어 채비 필수, 해뜨기 전후 입질 활발",
    },
}

# 어종 도감용 상세 정보 (습성/산란철/국내 이동경로)
FISH_GUIDE = {
    "참돔": {
        "습성": "평소 깊은 수심(50~100m)에 머물다 수온이 오르면 얕은 곳(20~40m)으로 접근. 무리를 짓기보다 개체·소규모로 활동하며 경계심이 강함",
        "산란철": "5~7월(수온이 오르는 초여름), 산란 직전·직후엔 얕은 연안으로 접근해 낚시 확률이 높아짐",
        "국내 이동경로": "겨울엔 제주 남쪽 등 따뜻한 심해에 머물다, 봄이 되면 수온을 따라 서해(군산·보령·인천)와 남해(여수·부산)로 북상. 가을~초겨울엔 다시 남쪽 깊은 수심으로 이동",
        "금어기": "전국 단위 지정 금어기 없음(2026.1.1 기준 수산자원관리법 시행령)",
    },
    "광어": {
        "습성": "장거리 회유 없이 특정 해역에 정착하는 저서성 어종. 모래·펄 바닥에 몸을 파묻고 위장해 지나가는 먹이를 습격",
        "산란철": "4~8월(주 산란기), 이 시기엔 살이 빠져 맛이 떨어지는 경우가 많음",
        "국내 이동경로": "먼 거리를 이동하기보다 수심 10~200m 범위 안에서 계절별로 얕은 곳과 깊은 곳을 오가는 정도. 서해·남해서부에 폭넓게 분포",
        "금어기": "전국 단위 지정 금어기 없음(2026.1.1 기준 수산자원관리법 시행령)",
    },
    "우럭": {
        "습성": "암초·인공어초 같은 구조물에 강하게 의존하는 정착성 어종, 한 자리에 오래 머무는 경향",
        "산란철": "겨울~초봄(난태생으로 늦가을 교미 후 이듬해 초 새끼를 낳음)",
        "국내 이동경로": "회유성이 약해 특정 포인트(구조물·어초)에 텃세를 갖고 머무는 경우가 많음. 서해·남해 방파제·근해에 고르게 분포",
        "금어기": "전국 단위 지정 금어기 없음(2026.1.1 기준 수산자원관리법 시행령)",
    },
    "문어": {
        "습성": "야행성이며 은신처(바위 틈, 통발)를 좋아함. 위장·색변화 능력이 뛰어나고 단독 생활",
        "산란철": "가을~초겨울에 짝짓기, 이후 암컷은 알을 지키다 생을 마감하는 습성",
        "국내 이동경로": "먼 거리 회유보다는 서식 암초 지대 내에서 이동. 가을철 씨알이 굵어지며 서해·남해 암초 지대에서 조황이 좋아짐",
        "금어기": "참문어(돌문어) 기준 5월 16일~6월 30일(기본, 시·도별로 다르게 고시될 수 있음)",
    },
    "주꾸미": {
        "습성": "펄 바닥에 서식하며 소라껍데기 등 은신처를 즐겨 씀, 산란기엔 소라 속에 알을 낳는 습성으로 유명",
        "산란철": "봄(3~5월)이 주 산란기, 이 시기 알배기 주꾸미가 인기",
        "국내 이동경로": "연안 얕은 수심에 머무는 경우가 많고, 서해 보령·태안 일대가 국내 최대 산지로 알려짐",
        "금어기": "5월 11일~8월 31일(전국)",
    },
    "갑오징어": {
        "습성": "먹물을 뿜어 위협에 대응, 몸 색을 바꿔 주변 환경에 위장하는 능력이 뛰어남",
        "산란철": "봄(4~6월)이 절정, 이 시기 연안 얕은 곳으로 산란을 위해 접근",
        "국내 이동경로": "겨울엔 깊은 곳에 머물다 봄에 산란을 위해 연안으로 이동, 서해·남해 전 연안에 폭넓게 분포",
        "금어기": "전국 단위 지정 금어기 없음(2026.1.1 기준 수산자원관리법 시행령)",
    },
    "갈치": {
        "습성": "표층~중층을 오가는 회유성 어종, 야간에 표층 가까이 올라오는 습성이 있어 야간 낚시가 발달",
        "산란철": "여름철(6~8월 전후)로 알려져 있으며, 정확한 산란장은 아직 조사가 진행 중",
        "국내 이동경로": "겨울엔 남쪽 제주 인근 깊은 바다에 머물다, 여름이 되면 남해안(여수·거문도)까지 북상하는 패턴을 보임",
        "금어기": "7월 1일~7월 31일(북위 33도 이북, 근해채낚기·연안복합어업은 제외돼 배낚시는 대부분 정상 운항)",
    },
    "농어": {
        "습성": "포식성이 강한 육식성 어종, 표층 가까이에서 먹이를 사냥하는 습성이 있어 루어에 반응이 좋음",
        "산란철": "가을~초겨울(수온이 내려가는 시기)",
        "국내 이동경로": "여름엔 하구·연안 얕은 곳으로 접근했다가, 겨울이 가까워지면 깊은 수심으로 이동하는 계절 회유",
        "금어기": "전국 단위 지정 금어기 없음(2026.1.1 기준 수산자원관리법 시행령)",
    },
    "백조기": {
        "습성": "무리를 지어 다니는 습성이 강해, 한 포인트를 잡으면 마릿수 조황으로 이어지는 경우가 많음",
        "산란철": "초여름(5~7월경)",
        "국내 이동경로": "서해 연안 모래·펄 바닥을 따라 무리지어 이동, 수온에 따라 얕은 곳과 깊은 곳을 오감",
        "금어기": "전국 단위 지정 금어기 없음(2026.1.1 기준 수산자원관리법 시행령)",
    },
    "대구": {
        "습성": "저수온을 선호하는 냉수성 어종, 깊은 수심에서 활동",
        "산란철": "겨울철(12~2월)이 대표 산란기",
        "국내 이동경로": "산란기가 되면 진해만 등 특정 연안 해역으로 회유해 들어오는 것으로 잘 알려져 있음. 평소엔 동해·남해 깊은 수심대에 서식",
        "금어기": "1월 16일~2월 15일(전국), 금지체장 35cm 이하",
    },
    "삼치": {
        "습성": "무리를 지어 빠르게 이동하는 회유성 등푸른 생선, 포식성이 강해 루어에 잘 반응",
        "산란철": "4~6월(주 산란기), 산란 후 가을부터 월동 준비로 살이 오름",
        "국내 이동경로": "봄~여름 산란을 위해 연안으로 접근했다가, 가을에 남하하며 서해 시화호·남해 전역에서 낚시 시즌을 형성",
        "금어기": "5월 1일~5월 31일(전국 기본), 인천 해역은 지역 시범사업으로 4월 10일~5월 10일로 조정",
    },
}
...
def fetch_tide_events(city: str, target_iso: str):
    ...
    # 'daily/YYYY-MM'은 "오늘" 기준 30일 롤링 목록이라 과거 날짜가 통째로 빠질 수 있다.
    # 정확한 날짜(YYYY-MM-DD)를 지정하면 그 날짜부터 목록이 시작되므로 이 문제를 피할 수 있다.
    url = f"https://www.badatime.com/{city_id}/daily/{target_date:%Y-%m-%d}"
    try:
    ...
    # 정확한 날짜(YYYY-MM-DD)로 요청했으므로 목록의 '첫 번째' 항목이 곧 그 날짜여야 정상이다.
    # 우선 첫 앵커의 요일이 실제로 일치하는지 검증하고, 다르면(사이트 구조 변경 등 예외 상황)
    # 예전 방식대로 날짜+요일이 일치하는 앵커를 찾아 대비한다.
    target_idx = None
    if anchors and int(anchors[0].group(1)) == target_date.day and anchors[0].group(2) == target_wd:
        target_idx = 0
    else:
...
with left:
    ...
    if fish in FISH_INFO:
        info = FISH_INFO[fish]
        with st.expander(f"🐟 {fish} 생태정보 보기"):
            for k, v in info.items():
                st.markdown(f"**{k}**: {v}")
            st.caption("참고용 정보이며, 지역·연도별 조황과 차이가 있을 수 있어요.")
    ...
    st.divider()
    with st.expander("📅 예약 등록"):
        r_date = st.date_input("날짜", value=date.today(), key="r_date")
        r_ship = st.text_input("선박", placeholder="예: 아시아호", key="r_ship")
        r_port = st.text_input("출항항", placeholder="예: 군산 비응항", key="r_port")
        r_time = st.text_input("예상시간 (선택)", placeholder="예: 5시-16시", key="r_time")
        r_anglers = st.multiselect("출조자", ANGLERS, key="r_anglers")
        r_payment = st.text_input("입금현황", placeholder="예: 입금, 입금0910", key="r_payment")
        r_memo = st.text_input("비고 (선택)", placeholder="예: 물세기33%, 260820 김정국추가", key="r_memo")
        r_account = st.text_input("계좌 (선택)", placeholder="예: 농협 3521550301163", key="r_account")
        r_amount = st.number_input("금액 (선택)", min_value=0, step=10000, value=0, key="r_amount")
        r_depositor = st.text_input("입금자명 (선택)", placeholder="예: 이형일", key="r_depositor")
        if st.button("예약 저장", key="r_save_btn"):
            if r_ship and r_anglers:
                reservations.append({
                    "date": r_date.strftime("%Y-%m-%d"),
                    "ship": r_ship, "port": r_port, "time": r_time,
                    "anglers": r_anglers, "payment": r_payment, "memo": r_memo,
                    "account": r_account, "amount": int(r_amount), "depositor": r_depositor,
                })
                save_json(RESERVATION_FILE, reservations)
                ok, msg = commit_to_github("reservations.json", reservations)
                if ok:
                    st.success(f"예약을 저장했습니다. {msg} 새로고침(F5) 하면 반영됩니다.")
                else:
                    st.warning(f"임시 저장은 됐지만 GitHub 자동 저장은 실패했어요: {msg}")
            else:
...
with right:
    ...
    with st.expander("📖 어종 도감"):
        ...
        for i, (sp, g) in enumerate(FISH_GUIDE.items()):
            with guide_cols[i % 2]:
                st.markdown(
                    f"<div style='background:#f8fafc;border:1px solid #e5e7eb;border-radius:12px;"
                    f"padding:12px 16px;margin-bottom:10px'>"
                    f"<div style='font-weight:800;color:#0b3b57;font-size:16px'>🐟 {sp}</div>"
                    f"<div style='font-size:13px;color:#33474f;margin-top:6px'><b>습성</b> — {g['습성']}</div>"
                    f"<div style='font-size:13px;color:#33474f;margin-top:4px'><b>산란철</b> — {g['산란철']}</div>"
                    f"<div style='font-size:13px;color:#33474f;margin-top:4px'><b>국내 이동경로</b> — {g['국내 이동경로']}</div>"
                    f"<div style='font-size:13px;margin-top:4px;color:{'#C1503F' if '없음' not in g.get('금어기','') else '#7a8794'}'><b>🚫 금어기</b> — {g.get('금어기','정보 없음')}</div>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
